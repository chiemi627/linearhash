    ...
    handleSubmit(){
      ...
      if(!cells.includes(value)){
          ...
          for(var i=0;i<slots[hashvalue].buckets.length;i++){
            ...
            for(var j=0;j<bucket.cells.length;j++){
              if(!bucket.cells[j]){
                ...
                else{slots[hashvalue].buckets[i].cells[j]=value;}
                break;
              }
            }
          }
        ...
        if(overflow){
          ...
          const hash_base = Math.pow(2,level+1)*this.state.initialslots;
          slots.push({buckets: [
                      {cells: Array(2).fill(null)}
                     ]});
